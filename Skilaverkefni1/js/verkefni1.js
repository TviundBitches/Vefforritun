    ...
    $("#load").click(function () {
        ...
        $.ajax({
            ...
            success: function(data) {
                var text = "";
                for(var i in data) {
                    text += '<a href="#!" class="collection-item" data-id="' + data[i].id + '">' + data[i].title + '</a>\n'
                }
                document.getElementById("images").innerHTML = text;
                $(".collection-item").click(function() {
                    $(".collection-item").removeClass("active");
                    $(this).addClass("active");
                    loadNr = $(this).attr("data-id");
    ...
    $("#tempSelect").click(function () {
        ...
        $.ajax({
            ...
            success: function(data) {
                var text = "";
                for(var i in data) {
                    text += '<a href="#!" class="collection-item" data-id="' + data[i].id + '">' + "Template " + i + '</a>\n'
                }
                document.getElementById("temps").innerHTML = text;
                $(".collection-item").click(function() {
                    $(".collection-item").removeClass("active");
                    $(this).addClass("active");
                    loadNr = $(this).attr("data-id");
                    $(".openTempsButton").click(function() {
                        $.ajax({
                            type: "GET",
                            url: url + "/" + loadNr,
                            success: function(data) {
